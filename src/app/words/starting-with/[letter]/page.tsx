...
import { getWordsStartingWith, getLetterCategorySEO, groupWordsByLength } from '@/lib/category-engine'
...
export async function generateMetadata({ params }: { params: { letter: string } }) {
  ...
  const seo = getLetterCategorySEO(letter, 'starts')
  return createPageMetadata({
    title: seo.title,
    description: seo.description,
    path: `/words/starting-with/${letter}`,
...
export default function WordsStartingWithPage({ params }: { params: { letter: string } }) {
  const { letter } = params
      ...
      <main className="pt-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
          ...
          <nav className="text-sm text-navy/50 font-body mb-3">
            ...
            <span className="mx-2">/</span>
            <span className="text-navy">Words Starting With {L}</span>
          </nav>
            ...
            return (
              <section key={len} className="mb-8">
                <h2 className="font-display font-bold text-navy text-xl mb-3 border-b border-gold/20 pb-2">
                  {len}-Letter Words Starting With {L}
          ...
          <div className="card mt-10 mb-8">
            <h2 className="font-display font-semibold text-navy text-xl mb-3">About Words Starting With {L}</h2>
          ...
          <div className="card mb-8">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {prevLetter && (
                <Link
                  href={`/words/starting-with/${prevLetter}`}
                  className="inline-flex items-center px-4 py-2 rounded-lg border border-navy/20 hover:border-gold/40 text-navy font-body text-sm transition-colors"
                >
                  ← Words Starting With {prevLetter.toUpperCase()}
                </Link>
              )}
              {nextLetter && (
                <Link
                  href={`/words/starting-with/${nextLetter}`}
                  className="inline-flex items-center px-4 py-2 rounded-lg border border-navy/20 hover:border-gold/40 text-navy font-body text-sm transition-colors"
                >
                  Words Starting With {nextLetter.toUpperCase()} →
                </Link>
              )}
          ...
          <RelatedTools currentPath={`/words/starting-with/${letter}`} />
        </div>
      </main>
      <Footer />
    </>
